...
import { Mail, Phone, MapPin, Instagram } from "lucide-react"; // TikTok removed previously
...
// Function to generate WhatsApp link (can be outside component)
const generateWhatsAppLink = (phoneNumber: string, message: string = "Hello, I'm interested in your services!"): string => {
      const encodedMessage = encodeURIComponent(message);
      return `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
};

export function ContactSection() {
  // Memoize the WhatsApp link generation
  const whatsappLink = useMemo(() => generateWhatsAppLink(contactInfo.phoneRaw), []);
             ...
             <div className="space-y-5 text-muted-foreground font-normal">
               ...
               <div className="space-y-3 pt-4 border-t border-border"> {/* Added spacing and border */}
                 ...
                 <div className="flex items-center space-x-4">
                    {/* WhatsApp SVG Icon */}
                    <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 transition-colors" aria-label="Chat on WhatsApp">
                    ...
                    {/* TikTok SVG Icon */}
                    <a href={contactInfo.tiktok} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 transition-colors" aria-label="Follow on TikTok">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
